...
5. Run a change detection test on the CCM to identify changes the stream
...
The signals are provided in 1-second i.i.d. clips of different classes (the nominal _interictal_ states where the patient is fine, and the non-nominal _ictal_ states where the patient is having a seizure), for each patient, and the original task of the challenge is to classify the clips correctly.  
...
Code for replicating our experiments will be published on [my Github](https://github.com/danielegrattarola)
soon.  
